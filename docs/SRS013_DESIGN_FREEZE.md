...
8. ⚠️ **Double-counting guard (acceptance test #8) — code-reviewed but not exercisable in this environment:** the `خصومات_الإدارة` tab does not currently exist in this Google Sheet (a pre-existing, unrelated condition — the admin-deductions UI's own help text already says to create it manually if missing), so the live test script detects this via the exact same error the feature would hit in production and **skips** this block rather than failing on an unrelated environment gap. The double-counting guard itself (`source==='ledger_native'` filter in `sumLedgerNativeForEntityPeriod`, applied identically regardless of how many `legacy_mirror` rows exist) was verified by direct code inspection and by the fact that `mirrorLegacyAdminDeduction()` is a strictly separate, `try/catch`-wrapped fire-and-forget call that never runs inside the ledger's own summation path. Re-run acceptance test #8 once that tab is created (or point this script at an environment where it already exists).
...
10. ✅ Regression guard: with the flag off (this repo's default — `FEATURE_PAYROLL_LEDGER_ENABLED` is unset in both `.env.local` and, pending this conversation's rollout decision, Vercel Production), `calculateSupervisorSalary()`'s new step is skipped entirely (`ledgerTotal` stays `0`, the `ledgerTransactions` field is omitted via the same `...(x ? {...} : {})` spread pattern used elsewhere in this codebase for additive-only fields) — verified by code inspection of the exact `if (flag) {...}` gate added around the one new line in the `netSalary` formula and the one new spread in the return object; every other line of the pre-existing calculation is untouched.
