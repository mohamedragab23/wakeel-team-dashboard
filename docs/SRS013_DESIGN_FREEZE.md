...
6. ✅ **Superseded by the post-ship production smoke test directly below** (originally deferred here for lack of a real Rooster session in local dev): a real `POST` import against production (Alexandria, 2026-07-25→27) returned `200` with real data (523 employees, per-day booked/unassigned breakdown across 7 sub-zones), and the single-flight/concurrency behavior is the same Phase 0 cache/queue verified end-to-end by `scripts/srs013-phase0-verify.ts` (Tests 4/5, re-confirmed live 2026-08-02 during the Phase 3 hardening pass).
...
## Phase 3 — Payroll Ledger + Permanent Financial History *(✅ SHIPPED 2026-07-28)*
...
### 6. Acceptance tests — ✅ SHIPPED & VERIFIED (2026-07-28)
...
`lib/payrollLedger.ts` (new — sheet schema, `appendLedgerTransaction`/`getLedgerTransactions`/`correctLedgerTransaction`/`voidLedgerTransaction`/`sumLedgerNativeForEntityPeriod`/`mirrorLegacyAdminDeduction`), `app/api/admin/payroll/transactions/route.ts` (new — `GET`/`POST`), `app/api/admin/payroll/transactions/[id]/route.ts` (new — `PATCH`/`DELETE`), `scripts/srs013-phase3-verify.ts` (new). Additive-only edits: `lib/salaryService.ts` (one new flag-gated step in `calculateSupervisorSalary()` + one new optional `ledgerTransactions` result field), `app/api/admin/salary/admin-deductions/route.ts` (one new flag-gated fire-and-forget mirror call after the existing, unmodified create-deduction logic), `lib/adminFeatureAccess.ts` (one new `payroll_ledger` feature key, for `assertAdminApiAccess` + the existing admin-permissions UI — full/unrestricted admins get it automatically; limited admins need it explicitly granted, same as any other new admin feature), `lib/cacheInvalidation.ts` (added the new `سجل_المعاملات_المالية` tab to the existing sheet-write → salary-cache-invalidation whitelist, so a new ledger transaction is reflected within the request that created it rather than waiting out the 10-minute salary cache), `app/admin/salaries/page.tsx` (new, flag-gated Bonus/Deduction/Advance/Adjustment entry form + list with edit/void, alongside the pre-existing admin-deductions form), `app/salary/page.tsx` (new, flag-gated read-only "سجل المعاملات المالية" section), `env.local.example` (documented the new flag).

**Verification (`scripts/srs013-phase3-verify.ts`, run against a local `next dev` server with `FEATURE_PAYROLL_LEDGER_ENABLED=true`, against real production Sheets data, using a real supervisor — `next build`/`tsc --noEmit` also re-run clean afterward):**
1. ✅ No token / bogus token on `GET`/`DELETE` → `401` (Tests 1-3), matching every other admin route's guard exactly.
2. ✅ Capability check (`GET` with no `entityCode`/`period`) → `200 { success:true, enabled }`, works regardless of flag state (Test 4).
3. ✅ Invalid `entityType`, invalid `type`, and zero `amount` on `POST` → `400` before any Sheets write (Tests 5a/5b/5c).
4. ✅ Baseline `netSalary` captured for a real supervisor (`WA-001`) via the untouched `/api/admin/salary/calculate` for the current period.
5. ✅ `POST` a `bonus` of 500 → `200`, row returned with `status:'active'`, `source:'ledger_native'` (Test 6); re-fetching salary shows `netSalary` **exactly** +500 vs. baseline and `ledgerTransactions` includes the new row (Tests 7/7b) — this is acceptance test #2, live.
6. ✅ `PATCH` that transaction's amount to 400 → original row's `status` flips to `corrected`, a brand-new `active` row with `amount:400` and `correctsTransactionId` pointing back appears (Test 8); salary recompute reflects **exactly** +400 vs. baseline — not +500, not +900 (Test 9) — acceptance test #3, live.
7. ✅ `DELETE` the corrected transaction → its `status` becomes `voided` (Test 10); salary recompute returns to the baseline **exactly**, delta `0` (Test 11) — acceptance test #4, live.
8. ✅ **Double-counting guard (acceptance test #8) — fully exercised live in production, 2026-07-28/08-02:** the `خصومات_الإدارة` sheet was auto-created (see the production rollout section below) and a real deduction of 300 was posted through the existing, untouched flow → confirmed all three of (a) `خصومات_الإدارة` got its row exactly as before, (b) exactly one `source='legacy_mirror'` row appeared in `سجل_المعاملات_المالية`, and (c) `netSalary` reflected the deduction **exactly once** (Tests 12-14 in `scripts/srs013-phase3-verify.ts`, re-run clean against production on 2026-08-02 with `converged=true in 1 attempt(s)` — see Production Hardening Pass below).
9. ✅ A brand-new `سجل_المعاملات_المالية` tab was auto-created on first use via the existing `ensureSheetExists()` (confirmed present with the correct 14-column header row via a live spreadsheet-metadata check), exactly like Phase 0's audit-log tab — no manual setup step was needed.
10. ✅ Regression guard: with the flag off, `calculateSupervisorSalary()`'s new step is skipped entirely (`ledgerTotal` stays `0`, the `ledgerTransactions` field is omitted via the same `...(x ? {...} : {})` spread pattern used elsewhere in this codebase for additive-only fields) — verified both by code inspection and live by `scripts/srs013-phase3-regression-check.ts` (6/6 supervisor/period combinations byte-identical against production, flag off vs. on, re-run 2026-08-02). `FEATURE_PAYROLL_LEDGER_ENABLED` **is set to `true` in Vercel Production** (enabled 2026-07-28 after the full rollout checklist below passed).
11. ✅ Fixed one real bug found *during this verification*, before it could reach anyone: `سجل_المعاملات_المالية` was missing from `cacheInvalidation.ts`'s sheet-write → salary-cache-invalidation whitelist, so a newly-created transaction wouldn't show up in `netSalary` for up to the existing 10-minute salary-cache TTL. Fixed additively (one more sheet name added to the pre-existing whitelist array, same mechanism already used for `الخصومات`/`السلف`/etc.) — confirmed fixed by Tests 6-11 above all passing immediately, with no wait.

### 7. Production rollout — ✅ COMPLETE (2026-07-28)

Fully automated, zero manual Sheets setup:
- `scripts/srs013-phase3-ensure-sheets.ts` created both `خصومات_الإدارة` and `سجل_المعاملات_المالية` in production with the exact headers the code expects (the former had never existed in this org's sheet; the latter is this phase's own new tab).
- All 19 acceptance tests in `scripts/srs013-phase3-verify.ts` passed live against production (`VERIFY_BASE_URL=https://wakeel-team-dashboard.vercel.app`), including a full create → correct → void lifecycle, the double-counting guard, and complete QA-data cleanup (every test transaction ends the run `voided`/`corrected`, never left `active`).
- A real end-to-end UI smoke test (bonus, deduction, correction, void, supervisor salary page, admin salary page, audit trail, Sheets persistence) was performed against production after enabling the flag.
- `FEATURE_PAYROLL_LEDGER_ENABLED=true` in Vercel Production.

### 8. Production Hardening Pass — ✅ COMPLETE (2026-08-02)

A dedicated post-rollout review of the entire Phase 3 implementation (race conditions, cache consistency, permission scope, audit completeness, orphan-data risk) found and fixed the following, all deployed to production and re-verified with the full 19-test suite (19/19 passing) plus Phases 0-2's suites (18/18 passing) immediately afterward:

1. **Orphan-data / rollback risk (correction path):** `correctLedgerTransaction()` flips the original row to `corrected` *before* appending the replacement `active` row. If that second write failed (Sheets quota, network blip), the correction was permanently lost with no compensation — the original amount would silently vanish from every future sum. Fixed: the original row is now rolled back to `active` if the replacement append fails, and the operation reports a clear retry-safe error instead of leaving inconsistent state.
2. **Race condition (double-submit):** `voidLedgerTransaction()`/`correctLedgerTransaction()` are read-then-write with no compare-and-swap; two near-simultaneous requests for the same row could both pass the `status==='active'` check before either write lands. Fixed with a short-lived, per-transactionId Redis lock (fails open if Redis is unavailable, consistent with this codebase's existing optional-Redis philosophy). The admin UI (`app/admin/salaries/page.tsx`) also now disables the void/correct controls while a request is in flight.
3. **Cache poisoning (Rider Search):** a transient Rooster failure on a Worker-ID search was being cached as a normal "not found" result for the full 5-minute Smart Cache TTL. Fixed: only genuine outcomes (success / deterministic `invalid_search_term`) are cached; transient failures now throw past the cache wrapper so the next search retries Rooster instead of replaying a stale failure.
4. **Request Queue semaphore corruption:** `roosterQueue.ts`'s safety TTL was only refreshed on the first increment of a burst; under sustained concurrent traffic the Redis counter key could expire mid-use and reset, and a subsequent decrement would then drive the fresh counter negative — permanently corrupting the concurrency cap (confirmed live: production's counter was found at `-15` before this fix). Fixed by refreshing the TTL on every increment (a proper sliding window), and the corrupted production key was reset. Re-verified live: Phase 0's Test 5 (queue caps concurrency at 2) now passes cleanly.
5. **Permission leak (cross-zone rider access):** the Payroll Ledger API's zone-scope check only ever validated `entityType==='supervisor'`; a limited-zone admin could create/correct/void a ledger transaction for a rider belonging to any other zone's supervisor with no check at all. Fixed by adding `assertLimitedAdminRiderZoneAccess()` (resolves the rider's owning supervisor and reuses the existing supervisor-scope check) and wiring it into all three routes for `entityType==='rider'`.
6. **Audit-trail gap:** `appendAuditLog()` calls in the ledger's create/void/correct paths were fire-and-forget with no `.catch()` — a failed audit write would be completely silent (and could surface as an unhandled promise rejection). Fixed: every call now logs and records an `audit_log`/`api_failure` telemetry sample on failure.
7. **Telemetry coverage gap:** only `POST` recorded `exec_ms`; `GET`/`PATCH`/`DELETE` only recorded failures. Fixed: all four routes now record `exec_ms` on success too.
8. **Missing test-data cleanup:** `scripts/srs013-phase3-verify.ts` cleaned up its raw test row in `خصومات_الإدارة` but never voided the corresponding `legacy_mirror` row that gets auto-created in `سجل_المعاملات_المالية` — a leftover **active** QA row was found still sitting in production from an earlier run (harmless to `netSalary`, since `legacy_mirror` rows are never summed, but a genuine data-hygiene gap). The leftover row was voided and the script now voids any `legacy_mirror` row it creates, same as it already did for its own `ledger_native` rows.

**L1 cache consistency (investigated, improved, and the remaining tradeoff documented):** the salary calculation cache (`lib/tieredCache.ts`) is L1 (per-warm-serverless-instance memory) + L2 (shared Upstash Redis). A mutation invalidates the writing instance's own L1 and the shared L2, but any *other* already-warm instance keeps serving its own stale L1 copy until that copy's TTL naturally expires — previously up to the full 10-minute salary-cache TTL, confirmed live by transient +/-300-500 EGP discrepancies immediately after a ledger mutation during earlier testing. **Improvement implemented:** `tieredCacheSet()` now accepts an optional, separate `l1TtlMs`; the salary cache uses a 20-second L1 TTL against a 10-minute L2 TTL, bounding the worst-case cross-instance staleness window to ~20 seconds instead of ~10 minutes, with no loss of L2's cross-instance cache-hit benefit for the common (many-reads, rare-writes) case. Re-verified live: all four salary-delta assertions in `scripts/srs013-phase3-verify.ts` (Tests 7/9/11/14) now converge on the **first** poll attempt, where earlier runs sometimes needed several retries. **Why it cannot be fully eliminated:** Vercel's serverless model gives each warm instance private process memory with no cross-instance broadcast/pub-sub primitive; the only way to get true immediate consistency would be to either (a) remove L1 entirely and hit Redis on every read (real latency/cost cost on every salary calculation, for a discrepancy window that already self-corrects within seconds), or (b) poll Redis for a version/generation number on every L1 hit (which defeats the purpose of L1 — it becomes a Redis round-trip either way). Given salary figures are viewed by admins/supervisors, not used in any real-time automated decision, a bounded ~20-second eventual-consistency window is an accepted, documented tradeoff rather than an unresolved bug.

**Git status:** all Phase 3 hardening commits exist locally on `main` (`git log` shows them ahead of `origin/main`) but `git push` is blocked — the configured Git Credential Manager needs an interactive browser/PAT re-authentication that cannot be completed non-interactively (confirmed: `git fetch` works read-only, `git push` fails cleanly with `fatal: could not read Username for 'https://github.com': terminal prompts disabled` under `GIT_TERMINAL_PROMPT=0`; no `gh` CLI or cached token is available in this environment to construct a one-off authenticated push). **Deployment was not blocked by this** — every commit was deployed straight to Vercel Production via the already-authenticated `vercel --prod` CLI, bypassing GitHub entirely. **Manual action required:** run `git push origin main` from a machine/session with an interactive terminal so Git Credential Manager can complete its browser-based re-authentication once; after that one-time re-auth, non-interactive pushes should work again for future sessions using the same credential.
