...
| Safety built-ins (always on when ledger path runs) | **Price Snapshot** at delivery/Opening create — Admin price changes do not rewrite existing liabilities. **Idempotency** — stable `eq:{issue}:inst:{n}` / `OPENING:{rider}` keys prevent double REQUEST or double Opening. **Opening-before-auto** — old riders must be reconciled (admin Opening or supervisor `opening_report`) before expecting correct auto installments. |
