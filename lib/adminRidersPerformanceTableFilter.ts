...
export type AdminPerfFilters = {
  code: TextFilterState;
  name: TextFilterState;
  supervisor: TextFilterState;
  region: TextFilterState;
...
export type AdminPerfSort = { col: string | null; dir: 'asc' | 'desc' };

export const ADMIN_PERF_NUM_KEYS = [
  'workDays',
  'hours',
  'break',
  'delay',
...
export type AdminPerfNumKey = (typeof ADMIN_PERF_NUM_KEYS)[number];
...
function getRankFilter(filters: AdminPerfFilters): {
  key: AdminPerfNumKey;
  mode: 'top' | 'bottom';
  n: number;
} | null {
  for (const key of ADMIN_PERF_NUM_KEYS) {
    const f = filters[key];
    if (f.op === 'top10') {
...
function numOf(r: AdminPerfRow, key: AdminPerfNumKey): number {
  switch (key) {
    case 'workDays':
      return Number(r.workDays) || 0;
    case 'hours':
...
export function compareAdminPerf(a: AdminPerfRow, b: AdminPerfRow, col: string, dir: 'asc' | 'desc'): number {
  const m = dir === 'asc' ? 1 : -1;
  switch (col) {
    case 'code':
    ...
    case 'supervisor': {
      const sa = `${a.supervisorName ?? ''} (${a.supervisorCode ?? ''})`.trim();
      const sb = `${b.supervisorName ?? ''} (${b.supervisorCode ?? ''})`.trim();
      return m * sa.localeCompare(sb, 'ar', { numeric: true });
    }
    case 'region':
      return m * (a.region || '').localeCompare(b.region || '', 'ar', { numeric: true });
    case 'date':
      return m * (a.date || '').localeCompare(b.date || '', 'ar', { numeric: true });
    case 'workDays':
...
export function applyAdminPerfFilters(rows: AdminPerfRow[], filters: AdminPerfFilters, sort: AdminPerfSort): AdminPerfRow[] {
  let out = rows.filter((r) => {
    if (!applyTextFilter(r.code ?? '', filters.code)) return false;
    if (!applyTextFilter(r.name ?? '', filters.name)) return false;
    const supCell = r.supervisorCode && r.supervisorCode.trim() !== '' ? `${r.supervisorName ?? ''} (${r.supervisorCode})` : '';
    if (!applyTextFilter(supCell, filters.supervisor)) return false;
    if (!applyTextFilter(r.region ?? '', filters.region)) return false;
    if (!applyTextFilter(r.date ?? '', filters.date)) return false;
    if (!applyNumScalar(Number(r.workDays) || 0, filters.workDays)) return false;
    ...
    if (!applyAbsenceFilter(r.absence ?? '', filters.absence)) return false;
  ...
  if (rank) {
    const sorted = [...out].sort((a, b) => {
      const va = numOf(a, rank.key);
      const vb = numOf(b, rank.key);
      return rank.mode === 'top' ? vb - va : va - vb;
    });
    out = sorted.slice(0, Math.min(rank.n, sorted.length));
  }

  if (sort.col) {
    out = [...out].sort((a, b) => compareAdminPerf(a, b, sort.col!, sort.dir));
  }
