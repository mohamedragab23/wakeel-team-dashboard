...
import { isGmailOAuthConfigured } from '@/lib/gmailOAuth';

export type FullRecoveryResult = { success: true; appCookieHeader: string } | { success: false; reason: string };

/** Both the Okta credential and the Gmail OTP-reading piece must be configured. */
export function isFullAuthRecoveryConfigured(): boolean {
  ...
  return hasOktaCreds && isGmailOAuthConfigured();
}
